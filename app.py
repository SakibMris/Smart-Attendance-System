...
def checkAttendance(id):
    ...
    query = query.where(attendance.c.time_date ==
                        date.today(), attendance.c.user_id == id)
    ...
    while True:
        ...
        if not success:
            ...
            for (top, right, bottom, left), name in zip(face_locations, face_names):
                ...
                cv2.putText(frame, "Status: "+status, (left + 6,
                            bottom+25), font, .5, (20, 128, 0), 1)
                cv2.rectangle(frame, (left, top - 30),
                              (right, top-1), (0, 0, 255), cv2.FILLED)
                cv2.putText(frame, insertStatus1, (left + 6, top-6),
                            font, .5, (255, 255, 255), 1)
